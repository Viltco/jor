...
class sh_back_theme_config_settings(models.Model):
    ...
    name = fields.Char(string="Theme Settings", compute='_compute_theme_name')

    @api.depends('theme_style')
    def _compute_theme_name(self):
        for rec in self:
            rec.name = rec.theme_style.replace('_', ' ').upper() if rec.theme_style else 'No Option Selected'
